...
import { menulistJsonData } from "./jsonData";
const CreateRolePage = () => {
  const { createRole, getRoleDetails, roleDetails, updateRole } =
    useRoleStore();
  ...
  const dynamicMenus = useMemo(() => {
    ...
    return [...(userDetails?.menuPermissions || [])].sort(
  ...
  const totalMenus = dynamicMenus.reduce(
    (acc, menu) => acc + (menu.childMenus?.length || 0),
    0,
  );

  const totalSelected = readCount + writeCount;

  const percentage = totalMenus
    ? Math.round((totalSelected / (totalMenus * 2)) * 100)
    : 0;
  ...
  const handleEnable = () => {
    const updatedValue = !enableAllRead;
    setEnableAllRead(updatedValue);
    handleEnableAllRead(updatedValue);
  };
  return (
    <div>
      <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-[4fr_1fr] gap-4">
        <div>
          {!isEdit && (
          ...
          <div className="flex flex-wrap justify-between border border-[#E6E7E8] bg-[#F9FAFB] p-4 rounded-xl my-7">
            ...
            <div>
              <div className="flex flex-wrap gap-2">
                <div className="flex gap-2 p-3 bg-[#F9FAFB] border border-[#E6E7E8] rounded-lg max-[500px]:mt-3">
                  {searchIcon}
                  <input
                    type="text"
        ...
        <div>
          ...
          <div className="mt-6 rounded-2xl border border-[#CDD0D1] p-5">
            ...
            <div className="flex justify-between border border-[#E2E8F0] rounded-2xl bg-[#F8FAFC] p-5 mt-7 max-[500px]:p-3 max-[500px]:text-[12px]">
              ...
              <ImageLoader
                imageKey={enableAllRead ? "openpassword" : "closeeyeIcon"}
